...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        rgb_map, depth_map, grad_loss = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray, N_samples=N_samples)

        rgbs.append(rgb_map)
        depth_maps.append(depth_map)
        eikonal_loss.append(grad_loss)
    
    return torch.cat(rgbs), None, torch.cat(depth_maps), None, None, eikonal_loss
